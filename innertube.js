...
async function getDeciphers() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!player_id) {
      reject("Could not find player id");
      return;
    }
...
(async function() {
    try {
        const info = await getDownloadStreams();
        window.webkit.messageHandlers.youtubeExtractor.postMessage(info);
    } catch (e) {
        window.webkit.messageHandlers.youtubeExtractor.postMessage(null);
    }
})();
